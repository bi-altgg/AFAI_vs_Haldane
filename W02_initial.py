...
for ky in ky_list:
    ...
    #Haldane
    lamdaH, V = eigu(U(n,Lx,T_H,ky,random))
...
for ny in range(Ly):
    ...
    for step in range(0, len(t_A)):
        ...
        if step == 0:
            GA[step,ny,:,:] = U(n, Lx, T_A, ky,random)
        else:
            GA[step,ny,:,:] = U(n, Lx,  (t_A[step] - t_A[step-1]), ky,random) @ GA[step - 1,ny,:,:]   # check for multiplication on the left vs right.
...
for ny in range(Ly):
    ...
    for step in range(0, len(t_H)):
        random = np.random.uniform(-W,W,(1,3))
        random_numbers_2.append(random)
        if step == 0:
            GH[step,ny,:,:] = U(n, Lx, T_H, ky,random)
        else:
            GH[step,ny,:,:] = U(n, Lx,  (t_H[step] - t_H[step-1]), ky,random) @ GH[step - 1,ny,:,:]

        
# Calculates the probability density for particle propogation in anomalous phase      
GprimeA= (np.fft.ifft(GA, axis = 1 ))
print(GprimeA.shape)
probA = np.abs(GprimeA[:,:,:,0] - GprimeA[:,:,:,1])**2
np.save('probA',probA)
probA_edge = np.sum(probA[:,0:5,:], axis = 1)
probA_bulk = np.sum(probA[:,5:45,:], axis = 1)
# Calculates the probability density for particle propogation in haldane phase      
GprimeH= (np.fft.ifft(GH, axis = 1 ))
probH = np.abs(GprimeH[:,:,:,0] - GprimeH[:,:,:,1])**2   
np.save('probH',probH)
probH_edge = np.sum(probH[:,0:5,:], axis = 1)
probH_bulk = np.sum(probH[:,5:45,:], axis = 1)
