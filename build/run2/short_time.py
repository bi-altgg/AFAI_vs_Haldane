...
def U(n, Lx, T, ky,noise):
    ...
    U_m = (V3 @ np.diag(np.exp(-1j*E3*(T*(1 + noise[0][0])/3))) @ V3.conj().T) @ \
       (V2 @ np.diag(np.exp(-1j*E2*(T*(1 + noise[0][1])/3))) @ V2.conj().T) @ \
       (V1 @ np.diag(np.exp(-1j*E1*(T*(1 + noise[0][2])/3))) @ V1.conj().T)
...
np.savetxt('probeHedge0_4_gauss_pre-avg-array.txt', prob_edge_average)
np.savetxt('probeHedge0_4_gaussavg.txt', average)
