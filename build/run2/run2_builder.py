...
#Do the git-ing
cmd='git commit -a -m "Commit before run run2" > /dev/null'
print(cmd)
...
for L in xrange(100):
	...
	task_file.write('bash run2_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('run2.sbatch.template','r').read()
...
open('run2.sbatch','w').write(contents)
